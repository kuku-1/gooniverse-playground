...
const GoonifyPage = () => {
  ...
  const goonStyles = [{
    id: "meme-lord",
    name: "Meme Lord",
    description: "Become the ultimate meme master with legendary status"
  }, {
    id: "retro-gamer",
    name: "Retro Gamer",
    description: "80s gaming nostalgia with pixel perfect aesthetics"
  }, {
    id: "ai-cyborg",
    name: "AI Cyborg",
    description: "Futuristic cyberpunk enhancement for maximum cool"
  }, {
    id: "pixel-goon",
    name: "Pixel Goon",
    description: "Classic 8-bit transformation for true arcade legends"
  }, {
    id: "goon-samurai",
    name: "Goon Samurai",
    description: "Honor, blade, and meme mastery combined"
  }];
  ...
  return <div className="min-h-screen pt-20 px-4 py-8">
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">Upload your selfie and transform into the ultimate goon   warrior. </p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <Card className="border-primary/20 bg-card/50 backdrop-blur-sm">
            ...
            <CardContent className="space-y-6">
              {/* Upload Area */}
              <div className="border-2 border-dashed border-primary/30 rounded-lg p-8 text-center cursor-pointer hover:border-primary/60 transition-colors" onClick={() => fileInputRef.current?.click()}>
                {previewUrl ? <div className="space-y-4">
                    <img src={previewUrl} alt="Preview" className="max-w-full max-h-64 mx-auto rounded-lg shadow-glow" />
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {goonStyles.map(style => <div key={style.id} className={`p-4 rounded-lg border cursor-pointer transition-all ${selectedStyle === style.id ? 'border-primary bg-primary/10 shadow-neon' : 'border-muted bg-card hover:border-primary/50'}`} onClick={() => setSelectedStyle(style.id)}>
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="font-orbitron font-bold text-foreground">{style.name}</h4>
                          <p className="text-sm text-muted-foreground">{style.description}</p>
                        </div>
                        {selectedStyle === style.id && <div className="w-4 h-4 rounded-full bg-primary animate-pulse"></div>}
                      </div>
                    </div>)}
                </div>
              </div>
              ...
              {/* Generate Button */}
              <Button variant="neon" size="lg" className="w-full text-lg py-6 h-auto" onClick={handleGoonify} disabled={!selectedFile || isGenerating}>
                {isGenerating ? <>
          ...
          <Card className="border-secondary/20 bg-card/50 backdrop-blur-sm">
            ...
            <CardContent className="space-y-6">
              {isGenerating ? <div className="flex flex-col items-center justify-center h-64 space-y-4">
                ...
                </div> : generatedImage ? <div className="space-y-6">
                  <div className="relative">
                    <img src={generatedImage} alt="Goonified" className="w-full rounded-lg shadow-epic" />
                    <Badge variant="secondary" className="absolute top-2 right-2 bg-gradient-secondary">
                      {goonStyles.find(s => s.id === selectedStyle)?.name}
                    </Badge>
                  </div>

                  <div className="space-y-3">
                    <Button variant="goon" size="lg" className="w-full text-lg py-4 h-auto" onClick={handleDownload}>
                      <Download className="w-5 h-5 mr-2" />
                      DOWNLOAD GOON IMAGE
                    </Button>

                    <Button variant="outline" size="lg" className="w-full text-lg py-4 h-auto">
                ...
                </div> : <div className="flex flex-col items-center justify-center h-64 space-y-4 text-center">
